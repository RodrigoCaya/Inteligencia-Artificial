...
int tope_examenes(struct Node *nodo_aux, struct Node *nodo, struct Stu **matriz_stu, int lines_stu, int *chequeo){
    ...
    if(nodo->child != NULL){
        return tope_examenes(nodo_aux, nodo->child, matriz_stu, lines_stu, chequeo);
    }
    return 0;
}
...
struct Orden** ordenar(struct Stu **matriz_stu, struct Exm **matriz_exm, struct Conf **matriz_conf, struct Orden **matriz_orden, char ***arreglo_solo_stu, int lines_stu, int lines_exm, int *cant_alumnos, int tipo_orden){
    ...
    if(tipo_orden == 1){
        matriz_conf = conflicto(matriz_stu, lines_stu, arreglo_solo_student, cant_al);
...
void vaciar_arreglo_fallo(int *arreglo_fallo, int lines_exm){
    int i;
    for(i=0 ; i<lines_exm ; i++){
        arreglo_fallo[i] = 0;
    }
...
struct Result*** grafo(struct Orden **matriz_orden, struct Stu **matriz_stu, struct CBJ **matriz_cbj, int lines_exm, int lines_stu, int *soluciones_totales, int timeslots, int printear){
    int i = 0, j, k, it, tope, cont, flag = 1, ultimo_tope = 0, soluciones = 0, look_back = 1, fallo = 0, num_solucion = 0, chequeoCBJ = 0, chequeo = 0, solucion_aux;
    ...
    int *arreglo_fallo = (int*)malloc(sizeof(int)*lines_exm);
    vaciar_arreglo_fallo(arreglo_fallo, lines_exm);
    j = 1;
    //inicializar nodo
    nodo->id_examen = 0;
    nodo->timeslot = 0;
    ...
    while(flag){
        ...
        for(i ; i<lines_exm ; i++){
            ...
            tope = tope_examenes(nodo_aux,nodo,matriz_stu,lines_stu,&chequeo);
            if(tope != 0 || j>timeslots){
                agregar_conf_cbj(matriz_cbj,lines_exm,nodo_aux->id_examen,tope);
                if(j <= timeslots && tope != 0){
                    arreglo_fallo[i] = arreglo_fallo[i] + 1;
                    cont = arreglo_fallo[i];
                    // printf("contador %d\n", cont);
                    if(cont == timeslots && look_back == 1){
                        printf("CBJ\n");
                        if(printear) chequeoCBJ++;
        ...
        if(printear){
            ...
            printf("   %d      |    %d     | %d %d %d\n",nodo->child->child->child->child->id_examen,nodo->child->child->child->child->timeslot,matriz_cbj[3]->id_examenes[0],matriz_cbj[3]->id_examenes[1],matriz_cbj[3]->id_examenes[2]);
        }
        // printf("fallo %d\n",fallo);
...
int contar_timeslots(struct Result** resultado, int lines_exm, int cant_timeslots){
    int i, j, flag, cont;
    int *timeslots = (int*)malloc(sizeof(int)*cant_timeslots);
    timeslots[0] = resultado[0]->timeslot;
    cont = 1;
    for(i = 0 ; i<lines_exm ; i++){
        flag = 0;
        for(j = 0 ; j<cont ; j++){
            if(resultado[i]->timeslot == timeslots[j]){
                flag = 1;
                break;
            }
        }
        if(flag == 0){
            timeslots[cont] = resultado[i]->timeslot;
            cont++;
        }
    }
    return cont;
...
int main() {
    ...
    //cuenta la cantidad de timeslots diferentes que utilizó la solución final
    if(decision == 1) cant_timeslots = contar_timeslots(resultado_final,lines_exm,cant_timeslots);
    //genera un output
